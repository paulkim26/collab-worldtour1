...
if callType == LuaCallType.Init then
    ...
    function startRotation(objectIndex, targetRot, targetSpeed, stopRotation)
        ...
        if not found then
            local rotation, speed, stopRotation, increment, time = parseInput(rotate[objectIndex].gameObject.GetComponent('Element').elementName)
            table.insert(objects, {
                index = objectIndex,
                originalRotation = rotate[objectIndex].transform.localRotation,
        ...
        for _, objData in ipairs(objects) do
            if objData.index == objectIndex then
                if objData.stopRotation then
                    rotate[objectIndex].transform.localRotation = Quaternion.Euler(objData.stopRotation.x, objData.stopRotation.y, objData.stopRotation.z)
                end
    ...
    function smoothRotate(objectData)
        ...
        if not objectData.increment then
            ...
            rotate[objectData.index].transform.localRotation = rotate[objectData.index].transform.localRotation * Quaternion.Euler(rotationX, rotationY, rotationZ)
        else
            ...
            rotate[objectData.index].transform.localRotation = Quaternion.Slerp(objectData.originalRotation, objectData.toRotation, clamp(elapsedTime, 0, 1))
            if elapsedTime >= 1 then
                objectData.isRotating = false
                objectData.slerpStarted = false
                rotate[objectData.index].transform.localRotation = objectData.toRotation
